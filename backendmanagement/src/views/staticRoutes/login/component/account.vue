...
// 动态登录成功后的页面跳转
const dynamicLogin = async () => {
	// 验证用户名与密码是否正确
	if (!state.ruleForm.username) return ElMessage.warning(t('请输入用户名'));
	if (!state.ruleForm.password) return ElMessage.warning(t('请输入密码'));
	if (!state.ruleForm.code) return ElMessage.warning(t('请输入验证码'));
	// 接入真实数据
	// 触发初始化用户信息 pinia
...
const onSignIn = async () => {
	...
	if (!themeConfig.value.isRequestRoutes) return await staticLogin();
	else return await dynamicLogin();
};
