...
// import { ezclient } from '/@/utils/ezcloudbase.js';
import ezInstance from '/@/utils/ezInstance.ts';// 引入ezcloud工具，用于执行数据操作  
...
const beforeVideoUpload = async (file: any, dialog: any, key = "video_id") => {
	...
	await ezclient.uploadVideo(file).then((res: any) => {
		console.log(res)
...
const beforeImageUpload = async (file: any, dialog: any, key = "img_id") => {
    // "method未传或传值有误，仅支持uimage、ufile、uvideo"
	console.log('file', file)
