	...
	<div class="system-banner-container layout-padding w100">
		<el-card shadow="hover" class="layout-padding-auto" style="height: 100%" body-class="el-card-tabel">
			<template #header>
				<div class="system-menu-search align-center">
					<div class="align-center">
						<span class="ml10" style="white-space: nowrap">资源名称：</span>
						<el-input size="default" clearable v-model="tableData.search" class="ml10" placeholder="请输入资源名称"
							style="max-width: 180px" @clear="getTableData">
						</el-input>
						<span class="ml10" style="white-space: nowrap">资源类型：</span>
						<el-select v-model="tableData.typeSearch" size="default" class="ml10" placeholder="请选择资源类型"
							@change="getTableData" style="width: 180px" clearable>
						...
						<el-button size="default" type="success" class="ml10" :icon="FolderAdd"
							@click="openDialogCurdFormRef({})"> 新增 </el-button>
					</div>
				</div>
			</template>
			<el-table :data="tableData.data" v-loading="tableData.loading" height="100%" style="width: 100%">
				...
				<el-table-column prop="dir" label="资源目录" />
				<el-table-column prop="title" label="配置标题" />
				<el-table-column prop="path" label="调整路径" />
				<el-table-column v-slot="scope" prop="img" label="内容" show-overflow-tooltip>
					<div v-if="scope.row.resource_type === '图片'">
						<el-image style="width: 80px; height: 80px; border-radius: 5%" :src="scope.row.img?.url"
					...
					<div v-else-if="scope.row.resource_type === '视频'">
						<el-image style="width: 80px; height: 80px; border-radius: 5%" :src="scope.row.video?.url"
							fit="cover" :preview-src-list="[scope.row.video?.url]" preview-teleported
							v-if="scope.row.video?.url" />
						<div v-else>-</div>
					</div>
					<div v-else-if="scope.row.resource_type === '文件'">

					</div>
					<div v-else-if="scope.row.resource_type === '富文本'">
		...
		<el-dialog :title="dialog.title" v-model="dialog.isShowDialog" width="60vw">
			<el-form ref="resourceFormRef" :model="dialog.ruleForm" :rules="dialog.rules" size="default"
				label-width="100">
				<el-row :gutter="50">
					...
					<el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="资源目录" prop="dir">
							<el-input v-model="dialog.ruleForm.dir" type="text" placeholder="请输入资源目录" clearable />
						</el-form-item>
					</el-col>
					<el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="配置标题" prop="title">
							<el-input v-model="dialog.ruleForm.title" type="text" placeholder="请输入配置标题" clearable />
						</el-form-item>
					</el-col>
					<el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12" class="mb20">
					...
					<el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="资源类型" prop="resource_type">
							<el-select v-model="dialog.ruleForm.resource_type" placeholder="请选择资源类型"
								style="width: 240px" @change="handleSelect">
								<el-option label="视频" value="视频" />
								<el-option label="图片" value="图片" />
								<el-option label="文件" value="文件" />
								<el-option label="富文本" value="富文本" />
					...
					<el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="调整路径" prop="path">
							<el-input v-model="dialog.ruleForm.path" type="text" placeholder="请输入点击时调整路径" clearable />
						</el-form-item>
					</el-col>
					<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
						<el-form-item label="图片" prop="img" v-if="dialog.ruleForm.resource_type === '图片'">
							<el-upload v-model:file-list="dialog.imageList" ref="upimage" action="#" :limit="1"
								...
								:on-remove="() => dialog.ruleForm.img_id = null" :on-preview="handlePictureCardPreview"
						...
						<el-form-item label="文件" prop="file" v-if="dialog.ruleForm.resource_type === '文件'">
							<el-upload v-model:file-list="dialog.fileList" ref="upfile" action="#" :limit="1"
								:auto-upload="true" :http-request="beforeLicenseHandle"
								:before-upload="async (file: any) => await beforeFileUpload(file, dialog, 'file_id')"
								:on-remove="() => dialog.ruleForm.file_id = null" :on-preview="handlePictureCardPreview"
								:on-exceed="(file: any) => handleExceed(upfile, file, 'file', dialog, 'file_id')"
								:accept="fileType.join(',')" style="width: 100%;">
								<template #trigger>
									<el-button type="primary">上传</el-button>
								</template>
							</el-upload>
						</el-form-item>
						<el-form-item label="富文本" prop="text" v-if="dialog.ruleForm.resource_type === '富文本'">
							<el-input v-model="dialog.ruleForm.text" type="textarea" placeholder="请输入富文本内容" />
						</el-form-item>
					</el-col>
				</el-row>
			</el-form>
...
const getTableData = async () => {
	...
	const { list, total_size } = await resourceFun.get(
		{
			...
			where: {
				...
				resource_type: tableData.typeSearch ? { _eq: tableData.typeSearch } : {}
			},
		},
		{}
	);
...
const openDialogCurdFormRef = (row: any = {}) => {
	...
	dialog.ruleForm = { // 确保所有字段都被正确赋值
		...row,
		resource_type: row.resource_type || '图片',
		img_id: row.img?.id ?? null,
		video_id: row.video?.id ?? null,
		file_id: row.file?.id ?? null,
		text: row.text || '',
		idx: row.idx || 1, // 确保 idx 字段有值
	};
	if (row.img?.id) {// 处理图片回显
		dialog.imageList = [{
			name: row.img.id,
			url: row.img.url
		}];
	}
	if (row.video?.id) { // 处理视频回显
		dialog.videoList = [{
			name: row.video.id,
			url: row.video.url
		}];
	}
	if (row.file?.id) {// 处理文件回显
		dialog.fileList = [{
			name: row.file.id,
			url: row.file.url
		}];
...
const handleSelect = () => {
	if (dialog.ruleForm.resource_type === '图片') {
		...
		dialog.ruleForm.file_id = null; // 清空文件
		dialog.ruleForm.text = ''; // 清空文本
		...
		dialog.ruleForm.file_id = null; // 清空文件
		dialog.ruleForm.text = ''; // 清空文本
	} else if (dialog.ruleForm.resource_type === '文件') {
		dialog.imageList = [];
		dialog.videoList = [];
		dialog.ruleForm.img_id = null; // 清空图片
		dialog.ruleForm.video_id = null; // 清空视频
		dialog.ruleForm.text = ''; // 清空文本
	} else if (dialog.ruleForm.resource_type === '富文本') {
		dialog.imageList = [];
		dialog.videoList = [];
		dialog.fileList = [];
		dialog.ruleForm.img_id = null; // 清空图片
...
const onSubmit = () => {
	...
	resourceFormRef.value.validate((valid: any) => {
		...
		if (dialog.ruleForm.resource_type === '文件' && !dialog.ruleForm.file_id) {
			ElMessage.error('请上传文件');
			return;
		}
		if (dialog.ruleForm.resource_type === '富文本' && !dialog.ruleForm.text) {
			ElMessage.error('请输入富文本内容');
			return;
		}
...
const onAdd = (ruleForm: any) => {
	ElMessageBox.confirm('确认新增该数据吗?', '提示', {
		...
		.then(async () => {
			const { id: ID, img, video, file, ..._ins } = ruleForm;
			await resourceFun
				.inc({ is_variable: true, ..._ins }, {})
				.then(() => {
					ElMessage({
						type: 'success',
						message: '添加成功！',
...
const onUpd = (ruleForm: any) => {
	ElMessageBox.confirm('确认修改该数据吗?', '提示', {
		...
		.then(async () => {
			const { id: ID, img, video, file, ..._set } = ruleForm;
			await resourceFun
				.set(
					ID,
					{
...
const onDel = (row: any) => {
	ElMessageBox.confirm('此操作将永久删除该数据, 是否继续?', '提示', {
		...
		.then(async () => {
			await resourceFun.del(row.id, false, {}).then(() => {
				ElMessage({
					type: 'success',
					message: '删除成功',
				});
