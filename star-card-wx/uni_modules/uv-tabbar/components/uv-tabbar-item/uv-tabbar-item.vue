<template>
	<view class="uv-tabbar-item" :style="[$uv.addStyle(customStyle)]" @tap="clickHandler">
		<view class="uv-tabbar-item__icon">
			<uv-icon v-if="icon" :name="icon" :color="isActive? parentData.activeColor : parentData.inactiveColor"
				:size="iconSize? iconSize: parentData.iconSize"></uv-icon>
			<template v-else>
				<slot v-if="isActive" name="active-icon" />
				<slot v-else name="inactive-icon" />
			</template>
			<uv-badge absolute :offset="[0, dot ? '34rpx' : badge > 9 ? '14rpx' : '20rpx']" :customStyle="badgeStyle"
				:isDot="dot" :value="badge || (dot ? 1 : null)" :show="dot || badge > 0"></uv-badge>
		</view>

		<slot name="text">
			<text class="uv-tabbar-item__text" :style="{
