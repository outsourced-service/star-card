<template>
	<view class="uv-navbar">
		<view class="uv-navbar__placeholder" v-if="fixed && placeholder" :style="{
		...
		<view :class="[fixed && 'uv-navbar--fixed']">
			...
			<uv-status-bar v-if="safeAreaInsetTop" :bgColor="getStatusbgColor"></uv-status-bar>
			<view class="uv-navbar__content" :class="[border && 'uv-border-bottom']" :style="[{
					height: $uv.addUnit(height)
				},getBgColor]">
				<view class="uv-navbar__content__left" hover-class="uv-navbar__content__left--hover"
					hover-start-time="150" @tap="leftClick">
					<slot name="left">
						<uv-icon v-if="leftIcon" :name="leftIcon" :size="leftIconSize" :color="leftIconColor"></uv-icon>
						<text v-if="leftText" :style="{
								color: leftIconColor
							}" class="uv-navbar__content__left__text">{{ leftText }}</text>
					</slot>
				</view>
				<slot name="center">
					<text class="uv-line-1 uv-navbar__content__title" :style="[{
							width: $uv.addUnit(titleWidth),
							flex: '0 1 auto'
						}, $uv.addStyle(titleStyle)]">{{ title }}</text>
				</slot>
				<view class="uv-navbar__content__right" @tap="rightClick">
					<slot name="right">
						<uv-icon v-if="rightIcon" :name="rightIcon" size="20"></uv-icon>
						<text v-if="rightText" class="uv-navbar__content__right__text">{{ rightText }}</text>
					</slot>
				</view>
			</view>
		</view>
